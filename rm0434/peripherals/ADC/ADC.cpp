...
// this
#include <rm0434/peripherals/ADC/ADC.hpp>

// xmcu
#include <rm0434/utils/delay.hpp>
                  ...
                  std::size_t a_group_size)
{
    ...
    {
        bit::flag::set(&(a_p_registers->CFGR), (a_group_size - 1) << ADC_CFGR_DISCNUM_Pos);
    }
    ...
    bit::flag::set(&(a_p_registers->ISR), ADC_ISR_EOS);

    bit::flag::set(&(a_p_registers->CR), ADC_CR_ADSTP);
    bit::flag::clear(&(a_p_registers->CR), ADC_CR_ADSTART);
}
...
void ADC::enable(Resolution a_resolution, const Channel* a_p_channels, std::size_t a_channels_count)
{
    ...
    hkm_assert(0x0u == bit::flag::get(this->p_registers->CR, ADC_CR_ADSTART | ADC_CR_JADSTART));

    bit::flag::clear(&(this->p_registers->CR), ADC_CR_DEEPPWD);
    bit::flag::set(&(this->p_registers->CR), ADC_CR_ADVREGEN);
    delay::wait(21_us);

    bit::flag::clear(&(this->p_registers->CR), ADC_CR_ADCALDIF);
    bit::flag::set(&(this->p_registers->CR), ADC_CR_ADCAL);

    wait_until::all_bits_are_cleared(this->p_registers->CR, ADC_CR_ADCAL);

    bit::flag::set(&(this->p_registers->CFGR), ADC_CFGR_RES_Msk, static_cast<std::uint32_t>(a_resolution));
    ...
    for (std::size_t i = 0; i < a_channels_count && i < 4; i++)
    {
        ...
        bit::flag::set(&(p_SQRs[0]), static_cast<uint32_t>(a_p_channels[i].id) << 6 * (i + 1));
    }

    for (std::size_t i = 4; i < a_channels_count; i++)
    {
        hkm_assert(various::get_enum_incorrect_value<Channel::Id>() != a_p_channels[i].id);
        bit::flag::set(&(p_SQRs[(i + 1) / 5]), static_cast<uint32_t>(a_p_channels[i].id) << 6 * ((i + 1) % 5));
    }
    ...
    for (std::uint32_t i = 0; i < a_channels_count; i++)
    {
        ...
        bit::flag::set(&(p_SMPRs[register_index]), sampling_time_val << ((channel_id - (register_index * 10)) * 3));
    }
    ...
    {
        bit::flag::set(&(ADC1_COMMON->CCR), ADC_CCR_TSEN);
        delay::wait(120_us);
    }
    ...
    {
        bit::flag::set(&(ADC1_COMMON->CCR), ADC_CCR_VBATEN);
    }
}
...
void ADC::disable()
{
    ...
    bit::flag::clear(&(ADC1_COMMON->CCR), ADC_CCR_TSEN | ADC_CCR_VREFEN | ADC_CCR_VBATEN);

    bit::flag::set(&(this->p_registers->CR), ADC_CR_ADDIS);
    wait_until::all_bits_are_cleared(this->p_registers->CR, ADC_CR_ADDIS);

    bit::flag::set(&(this->p_registers->CR), ADC_CR_DEEPPWD);
}
...
ADC::Resolution ADC::get_resolution() const
{
    ...
    return static_cast<Resolution>(bit::flag::get(this->p_registers->CFGR, ADC_CFGR_RES_Msk));
}

std::array<std::pair<ADC::Channel::Id, bool>, ADC::s::max_channels_count> ADC::get_enabled_channels()
{
    std::uint32_t enabled_channels_count = bit::flag::get(this->p_registers->SQR1, ADC_SQR1_L) + 1;
    ...
    for (std::uint32_t i = 0; i < enabled_channels_count && i < 4; i++)
    {
        ...
        const std::uint32_t index = bit::flag::get(p_SQRs[0], 0x1Fu << offset) >> offset;
    ...
    for (std::uint32_t i = 4; i < enabled_channels_count; i++)
    {
        ...
        const std::uint32_t index = bit::flag::get(p_SQRs[(i + 1u) / 5u], 0x1Fu << offset) >> offset;
        ret[index].second = true;
    }
...
template<> void ADC::Interrupt::read_start<ADC::Mode::single>(const Callback& a_callback)
{
    ...
    bit::flag::clear(&(this->p_ADC->p_registers->CFGR), ADC_CFGR_CONT);
...
template<> void ADC::Interrupt::read_start<ADC::Mode::continuous>(const Callback& a_callback)
{
    ...
    bit::flag::set(&(this->p_ADC->p_registers->CFGR), ADC_CFGR_CONT);
    bit::flag::set(&(this->p_ADC->p_registers->IER), ADC_IER_EOCIE | ADC_IER_EOSIE);
    bit::flag::set(&(this->p_ADC->p_registers->CR), ADC_CR_ADSTART);
}
template<>
void ADC::Interrupt::read_start<ADC::Mode::discontinuous>(const Callback& a_callback, std::size_t a_group_size)
{
    ...
    bit::flag::set(&(this->p_ADC->p_registers->CFGR), (a_group_size - 1) << ADC_CFGR_DISCNUM_Pos);
...
void ADC::Interrupt::read_stop()
{
    ...
    bit::flag::clear(&(this->p_ADC->p_registers->CFGR), ADC_CFGR_CONT | ADC_CFGR_DISCEN | ADC_CFGR_DISCNUM_Msk);
...
{
    bit::flag::set(&(RCC->AHB2ENR), RCC_AHB2ENR_ADCEN);
    bit::flag::set(&(RCC->CCIPR), RCC_CCIPR_ADCSEL_Msk);
    bit::flag::set(
        &(ADC1_COMMON->CCR), ADC_CCR_CKMODE_Msk | ADC_CCR_PRESC_Msk, static_cast<std::uint32_t>(a_prescaler));
}
template<> void rcc<ADC>::async::enable<pll::sai1::r>(Prescaler a_prescaler, bool a_enable_in_lp)
{
    bit::flag::set(&(RCC->AHB2ENR), RCC_AHB2ENR_ADCEN);
    bit::flag::set(&(RCC->CCIPR), RCC_CCIPR_ADCSEL_Msk, RCC_CCIPR_ADCSEL_0);
    bit::flag::set(
        &(ADC1_COMMON->CCR), ADC_CCR_CKMODE_Msk | ADC_CCR_PRESC_Msk, static_cast<std::uint32_t>(a_prescaler));
}
...
{
    bit::flag::set(&(RCC->AHB2ENR), RCC_AHB2ENR_ADCEN);
    bit::flag::set(&(RCC->CCIPR), RCC_CCIPR_ADCSEL_Msk, RCC_CCIPR_ADCSEL_1);
    bit::flag::set(
        &(ADC1_COMMON->CCR), ADC_CCR_CKMODE_Msk | ADC_CCR_PRESC_Msk, static_cast<std::uint32_t>(a_prescaler));
}
