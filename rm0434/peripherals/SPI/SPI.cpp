...
// this
#include <rm0434/peripherals/SPI/SPI.hpp>
...
void enable(SPI_TypeDef* a_p_registers)
{
    ...
    bit::flag::set(&a_p_registers->CR1, SPI_CR1_LSBFIRST, 0);                  // MSBit first

    bit::flag::set(&a_p_registers->CR2, SPI_CR2_DS, SPI_CR2_DS_0 | SPI_CR2_DS_1 | SPI_CR2_DS_2); // 8-bit transfers
    bit::flag::set(&a_p_registers->CR2, SPI_CR2_SSOE, 0);              // hw CS signal disabled - multi-CS or other pin
    bit::flag::set(&a_p_registers->CR2, SPI_CR2_FRXTH, SPI_CR2_FRXTH); // trigger RXNE on first byte written
...
SPI::Polling::Result receive(SPI_TypeDef* a_p_registers, data_t* a_p_data, std::size_t a_data_size_in_words)
{
    ...
    while (0 != bit::flag::get(a_p_registers->SR, SPI_SR_FRLVL))
    {
        *a_p_data = *p_data_reg;
    }
